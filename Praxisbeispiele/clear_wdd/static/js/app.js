        ...
        switch(event.type) {
            ...
            case 'dragright':
                ...
                if (event.gesture.deltaX > 70) {
                    el.style.backgroundColor = 'rgb(0, 167, 50)';
                } else if (event.gesture.deltaX < 70) {
                    // Note: might be bad for performance
                    el.style.backgroundColor = el.getAttribute('data-bg');
                }
                break;
            case 'release':
                if (event.gesture.deltaX >= 0 && event.gesture.deltaX < 90) {
                    el.style.webkitTransform = '';
                    el.style.backgroundColor = el.getAttribute('data-bg');
                } else if (event.gesture.deltaX > 90) {
                    Hammer(el.parentNode).off('dragleft');
                    doneTodo(el.parentNode);

                    var done_todo = el.parentNode;
                    todoList.removeChild(el.parentNode);
                    done_todo.classList.add('done');
                    var done_div = done_todo.querySelector('div');
                    done_div.style.webkitTransform = '';
                    done_div.style.backgroundColor = 'transparent';
                    todoList.appendChild(done_todo);
                }
                break;
        }
    }
